...
const EnumEquals = (enumeration, other) => {
  const map = RequireInternalSlot(enumeration, $Map);
  if (enumeration === other) {
    return true;
  }
  if (!IsObject(other)) {
    return false;
  }
  const otherMap = GetInternalSlot(other, $Map);
  if (!otherMap) {
    return false;
  }
  const size = MapSize(map);
  const otherSize = MapSize(otherMap);
  if (size !== otherSize) {
    return false;
  }
  const iterator = MapEntries(map);
  const otherIterator = MapEntries(otherMap);
  let iteratorResult = MapIteratorNext(iterator);
  let otherIteratorResult = MapIteratorNext(otherIterator);
  while (!iteratorResult.done) {
    const entry = iteratorResult.value;
    const otherEntry = otherIteratorResult.value;
    const key = entry[0];
    const otherKey = otherEntry[0];
    if (key !== otherKey) {
      return false;
    }
    const value = entry[1];
    const otherValue = otherEntry[1];
    if (value !== value) {
    ...
    iteratorResult = MapIteratorNext(iterator);
    otherIteratorResult = MapIteratorNext(otherIterator);
  }
  return true;
}

const EnumForEach = (enumeration, callback) => {
  const map = RequireInternalSlot(enumeration, $Map);
  ...
  while (!iteratorResult.done) {
    ...
    const key = entry[0];
    const value = entry[1];
    ReflectApply(callback, null, [value, key, enumeration]);
    iteratorResult = MapIteratorNext(iterator);
  }
}
